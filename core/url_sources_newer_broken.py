...
class DocumentationURLCollector:
    ...
    def _validate_samples_cache(self) -> bool:
        ...
        try:
            ...
            if 'samples' not in cache_data:
                logger.warning("No samples in cache")
                return False
                
            if not cache_data.get('cached_at'):
                logger.warning("No cache timestamp")
                return False
    ...
    def _update_cache(self, samples: List[ProjectResource]):
        ...
        try:
            ...
            self.samples_cache.parent.mkdir(parents=True, exist_ok=True)
    ...
    def clear_cache(self):
        ...
        for cache_file in [self.samples_cache, self.url_cache, self.doc_cache, self.analysis_cache]:
    ...
    def inspect_cache(self):
        ...
        if self.samples_cache.exists():
            try:
                samples = json.loads(self.samples_cache.read_text())
                sample_items = samples.get('samples', [])
                downloaded = [s for s in sample_items if s.get('downloaded')]
                
                logger.info("\nSamples Cache:")
                logger.info(f"Total samples: {len(sample_items)}")
                logger.info(f"Downloaded: {len(downloaded)}")
                logger.info(f"Not downloaded: {len(sample_items) - len(downloaded)}")
